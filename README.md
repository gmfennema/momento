...
1. **Generator** (`/`): upload or record audio → conditioned for the codec
   ...
   **card front**: the Momento mark, the clip's waveform as a mirrored bar
   chart, the name line, and a "scan the back to listen" nudge — downloadable
   as its own PNG/SVG pair.
